...
export type EnvkitLogger = {
  ...
  error: (message: string, error?: Error | KitiumError, metadata?: Record<string, unknown>) => void;
 ...
 */
export function getEnvkitLogger(context: Record<string, unknown> = {}): EnvkitLogger {
  ...
  return {
    info: (message: string, metadata?: Record<string, unknown>): void => {
      contextualLogger.info(`${prefix} ${message}`, { ...context, ...metadata });
    },
    warn: (message: string, metadata?: Record<string, unknown>): void => {
      contextualLogger.warn(`${prefix} ${message}`, { ...context, ...metadata });
    },
    error: (
      message: string,
      ...
      if (error) {
        contextualLogger.error(`${prefix} ${message}`, { ...context, ...metadata }, error);
      } else {
        contextualLogger.error(`${prefix} ${message}`, { ...context, ...metadata });
      }
    },
    debug: (message: string, metadata?: Record<string, unknown>): void => {
      contextualLogger.debug(`${prefix} ${message}`, { ...context, ...metadata });
    },
  };
}
...
export function createEnvkitError(
  ...
  const errorShape = {
    ...
    ...(options?.context && { context: { ...options.context } as ErrorContext }),
  };
